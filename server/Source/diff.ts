...
export class LcsDiff {
	...
	private ElementsAreEqual(originalIndex: number, newIndex: number): boolean {
		if (
			this._originalElementsOrHash[originalIndex] !==
			this._modifiedElementsOrHash[newIndex]
		) {
			return false;
		}
	...
	private OriginalElementsAreEqual(index1: number, index2: number): boolean {
		if (
			this._originalElementsOrHash[index1] !==
			this._originalElementsOrHash[index2]
		) {
			return false;
		}
	...
	private ModifiedElementsAreEqual(index1: number, index2: number): boolean {
		if (
			this._modifiedElementsOrHash[index1] !==
			this._modifiedElementsOrHash[index2]
		) {
			return false;
		}
				...
				if (
					...
					if (originalIndex >= reversePoints[diagonal]) {
						...
						if (
							...
							numDifferences <=
								LocalConstants.MaxDifferencesHistory + 1
						) {
	...
	private PrettifyChanges(changes: DiffChange[]): DiffChange[] {
		...
		for (let i = changes.length - 1; i >= 0; i--) {
			...
			for (let delta = 1; ; delta++) {
				...
				const score = this._boundaryScore(
					originalStart,
					change.originalLength,
					modifiedStart,
					change.modifiedLength,
	...
	private ChangesOverlap(
		left: DiffChange,
		right: DiffChange,
		mergedChangeArr: Array<DiffChange | null>,
	): boolean {
		Debug.Assert(
