...
namespace CodeActionsOnSave {
	export function isExplicitlyDisabled(
		setting: CodeActionsOnSave | undefined,
	): boolean {
		if (
			setting === undefined ||
			setting === null ||
	...
	export function getSourceFixAll(
		setting: CodeActionsOnSave,
	): boolean | undefined {
		if (setting === null) {
			return undefined;
		}
	...
	export function getSourceFixAllESLint(
		setting: CodeActionsOnSave,
	): boolean | undefined {
		if (setting === null) {
			return undefined;
	...
	export function setSourceFixAllESLint(
		setting: CodeActionsOnSave,
...
namespace MigrationData {
	export function create<T>(
		inspect: InspectData<T> | undefined,
	): MigrationData<T> {
...
export class Migration {
	...
	private recordAutoFixOnSave(): [boolean, boolean, boolean] {
		function record(
			this: void,
			elem: MigrationElement<boolean>,
			setting: MigrationElement<CodeActionsOnSave>,
		): boolean {
			...
			if (
				...
				autoFix &&
				CodeActionsOnSave.getSourceFixAllESLint(setting.value) ===
					undefined
			) {
				CodeActionsOnSave.setSourceFixAllESLint(
					setting.value,
					elem.value,
				);
		...
		return [
			...
			record(
				this.autoFixOnSave.workspaceFolder,
				this.codeActionOnSave.workspaceFolder,
			),
		];
	}
	...
	private recordValidate(fixAll: [boolean, boolean, boolean]): void {
		function record(
			this: void,
			elem: MigrationElement<(ValidateItem | string)[]>,
			settingAccessor: (
				language: string,
			) => MigrationElement<CodeActionsOnSave>,
			fixAll: boolean,
		): void {
		...
		record(
			...
			fixAll[2] ? fixAll[2] : fixAll[1] ? fixAll[1] : fixAll[0],
		);
	}
	...
	public needsUpdate(): boolean {
		if (
			MigrationData.needsUpdate(this.autoFixOnSave) ||
	...
	public async update(): Promise<void> {
		async function _update<T>(
			config: WorkspaceConfiguration,
			section: string,
			newValue: MigrationElement<T>,
		...
		async function _updateLanguageSetting(
			config: WorkspaceConfiguration,
			section: string,
			settings: LanguageSettings | undefined,
			newValue: MigrationElement<CodeActionsOnSave>,
			target: ConfigurationTarget,
		): Promise<void> {
			if (!newValue.changed) {
				return;
		...
		try {
			...
			await _update(
				this.eslintConfig,
				"workingDirectories",
				this.workingDirectories.workspaceFolder,
				ConfigurationTarget.WorkspaceFolder,
			);
