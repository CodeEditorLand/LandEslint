...
import {
	...
	TextDocument,
	Uri,
	DocumentFilter as VDocumentFilter,
	window as Window,
...
export class Validator {
	...
	public check(textDocument: TextDocument): Validate {
		...
		if (
			...
			config.get<boolean>("ignoreUntitled", false)
		) {
			return Validate.off;
		}
...
export namespace ESLintClient {
	...
	namespace PerformanceStatus {
		export const defaultValue: PerformanceStatus = {
			firstReport: true,
			validationTime: 0,
			fixTime: 0,
			reported: 0,
	...
	export function create(
		...
		const packageJsonFilter: VDocumentFilter = {
			scheme: "file",
		...
		const supportedQuickFixKinds: Set<string> = new Set([
			...
			`${CodeActionKind.SourceFixAll.value}.eslint`,
		...
		const client: LanguageClient = new LanguageClient(
			"ESLint",
			createServerOptions(context.extensionUri),
			createClientOptions(),
		);
			...
			Workspace.onDidChangeConfiguration(() => {
				...
				for (const textDocument of syncedDocuments.values()) {
					if (validator.check(textDocument) === Validate.off) {
						...
						provider
							...
							.catch((error) =>
								client.error(
									`Sending close notification failed.`,
									error,
								),
							);
					}
				...
				for (const textDocument of Workspace.textDocuments) {
					if (
						...
						validator.check(textDocument) !== Validate.off
					) {
						const provider = client
						...
						provider
							...
							.catch((error) =>
								client.error(
									`Sending open notification failed.`,
									error,
								),
							);
					}
		...
		function createServerOptions(extensionUri: Uri): ServerOptions {
			...
			const result: ServerOptions = {
				run: {
					...
					runtime,
					options: { execArgv, cwd: process.cwd(), env },
				},
				debug: {
					module: serverModule,
					transport: TransportKind.ipc,
			...
			const clientOptions: LanguageClientOptions = {
				...
				diagnosticPullOptions: {
					...
					filter: (document, mode) => {
						...
						if (
							...
							mode === DiagnosticPullMode.onSave &&
							run !== "onSave"
						) {
							return true;
						}
				...
				middleware: {
					...
					didChange: async (event, next) => {
						if (
							syncedDocuments.has(event.document.uri.toString())
						) {
							return next(event);
						}
					},
					...
					notebooks: {
						...
						didChange: (event, next) => {
							if (event.cells?.structure?.didOpen !== undefined) {
								for (const open of event.cells.structure
									.didOpen) {
									syncedDocuments.set(
							...
							if (
								...
								for (const closed of event.cells.structure
									.didClose) {
									syncedDocuments.delete(
										closed.document.uri.toString(),
									);
								}
							}
		...
		async function getPackageManager(uri: Uri) {
			const userProvidedPackageManager: PackageManagers =
				Workspace.getConfiguration("eslint", uri).get(
					"packageManager",
					"npm",
				);
		...
		async function readConfiguration(
			...
			for (const item of params.items) {
								...
								const selected =
									await Window.showInformationMessage<Item>(
										[
											...
											folder !== undefined
												? `for the workspace folder: ${folder}.`
												: `for the file: ${file}.`,
				...
				const settings: ConfigurationSettings = {
					...
					useESLintClass: config.get<boolean>(
						"useESLintClass",
						false,
					),
					useFlatConfig:
						useFlatConfig === null ? undefined : useFlatConfig,
					experimental: {
						useFlatConfig: config.get<boolean>(
					...
					onIgnoredFiles: ESLintSeverity.from(
						config.get<string>(
							"onIgnoredFiles",
							ESLintSeverity.off,
						),
					...
					rulesCustomizations: getRuleCustomizations(
						config,
						resource,
					),
					run: config.get<RunValues>("run", "onType"),
					problems: {
						shortenToSingleLine: config.get<boolean>(
							"problems.shortenToSingleLine",
							false,
						),
					},
					nodePath:
						config.get<string | undefined>("nodePath", undefined) ??
						null,
					workingDirectory: undefined,
					workspaceFolder: undefined,
					codeAction: {
						disableRuleComment: config.get<
							CodeActionSettings["disableRuleComment"]
						>("codeAction.disableRuleComment", {
							enable: true,
							location: "separateLine" as const,
							commentStyle: "line" as const,
						}),
						showDocumentation: config.get<
							CodeActionSettings["showDocumentation"]
					...
					for (const entry of workingDirectories) {
								...
								if (directory !== undefined) {
									...
									if (
										...
										pattern = path.posix.join(
											toPosixPath(workspaceFolderPath),
											pattern,
										);
									}
						...
						if (itemValue !== undefined) {
							if (
								...
								if (
									workingDirectory.directory.length <
									itemValue.length
								) {
		...
		function parseRulesCustomizations(
			rawConfig: unknown,
		): RuleCustomization[] {
			...
			return rawConfig
				.map((rawValue) => {
					if (
						typeof rawValue.severity === "string" &&
						typeof rawValue.rule === "string"
					) {
		...
		function getRuleCustomizations(
		...
		function updateStatusBar(textDocument: TextDocument | undefined) {
			...
			let validationBudget = Workspace.getConfiguration(
				"eslint",
				activeTextDocument,
			).get<TimeBudget>("timeBudget.onValidation", {
				warn: 4000,
				error: 8000,
			});
			...
			if (validationBudget.warn < 0 || validationBudget.error < 0) {
				validationBudget = {
					warn:
						validationBudget.warn < 0
							? Number.MAX_VALUE
							: validationBudget.warn,
					error:
						validationBudget.error < 0
							? Number.MAX_VALUE
			...
			let fixesBudget = Workspace.getConfiguration(
				"eslint",
				activeTextDocument,
			).get<TimeBudget>("timeBudget.onFixes", {
				warn: 3000,
				error: 6000,
			});
			...
			const [timeTaken, detail, message, timeBudget] = (function (): [
				...
				if (
					performanceInfo === undefined ||
					performanceInfo.firstReport ||
					performanceInfo.acknowledged
				) {
