...
(async function main() {
	const languageServerDirectory = path.normalize(
		path.join(root, "../../../LanguageServer/Node"),
	);

	console.log(
		`Symlinking to language server libs at location ${languageServerDirectory}`,
	);
	...
	await ln.tryHardLink(
		path.join(languageServerDirectory, "client"),
		path.join(root, "client", "node_modules", "vscode-languageclient"),
	);
